...
const callback = () => { };
function AuthomatlyInsertCollaborator_Dimensioning() {
    poolAlmaverso.getConnection((errAlmaverso, connectionAlmaverso) => {
        ...
        const selectDataFromShifts = `SELECT Documento, Fecha, Turno_Ini, Turno_Fin FROM T_Shifts WHERE Fecha >= '${DataAtualizado}'`;
                    ...
                    try {
                        ...
                        connection.query(insertDataCollaborator_Dimensioning, [values], (errInsert, resultInsert) => {
                            ...
                            if (errInsert) {
                                ...
                                console.log('No se puedo insertar los turnos');
                                callback('No se puedo insertar los turnos');
                            } else {
                                console.log('listo el pollo 2');
...
function scheduleAutomaticRejection() {
    ...
    setTimeout(() => {
        AuthomatlyInsertCollaborator_Dimensioning();
        scheduleAutomaticRejection();
    }, millisecondsUntilMidnight);
}
scheduleAutomaticRejection();

module.exports = AuthomatlyInsertCollaborator_Dimensioning;
