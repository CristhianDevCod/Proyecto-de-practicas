...
const AthomathlyUpdateTablesStaffShifts = express();
const mysql = require('mysql2');
const cors = require('cors');

AthomathlyUpdateTablesStaffShifts.use(cors());
AthomathlyUpdateTablesStaffShifts.use(express.json());
AthomathlyUpdateTablesStaffShifts.use(express.urlencoded({ extended: false }));
...
// Función para verificar cambios en los turnos de staff y actualizar sistema de puntos si es necesario
const checkForChangesAndUpdate = async () => {
    try {
        ...
        const selectQuery = `SELECT Documento, Fecha, Turno_Ini, Turno_Fin, Novedad FROM T_Shifts_Staff WHERE Fecha >= '${Fecha}'`;
        ...
        if (!arraysAreEqual(lastShiftsData, newShiftsData)) {
            ...
            callback('No se detectaron cambios en la tabla de staff');
        }
    } catch (error) {
        callback('Error al verificar cambios en la tabla staff:', error);
    }
};
...
// Función para actualizar otras tablas según los cambios en la tabla de staff
...
module.exports = AthomathlyUpdateTablesStaffShifts;
