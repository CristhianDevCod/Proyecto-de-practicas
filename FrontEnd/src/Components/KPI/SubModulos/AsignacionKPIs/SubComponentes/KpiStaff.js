...
import { getAreas } from "../../../../ReusablesModulo/TableKpi/columns.js";
...
const KpiStaff = () => {
    ...
    const filtroStaff = Array.isArray(staff)
        ? staff.filter((item) => item.areaNombre.toLowerCase().includes(searchStaff.toLocaleLowerCase()) || item.procesoNombre.toLowerCase().includes(searchStaff.toLowerCase()))
        : {};
    ...
    const getAll = useCallback(async () => {
        setLoading(true)
        try {
            const responseStaff = await GETALLAREASPROCESO();
            setStaff(responseStaff);
        } catch (error) {
    ...
    // Ver KPIS asignados
    const verArea = async (row) => {
        try {
            //Llamada al endpoint para obtener todos los KPIs
            const response = await apiClient.get(`http://${Servidor}/API/GET-ALL-KPIS-AREAS/`);
            const kpisRelacionados = Array.isArray(response.data)
                ? response.data.filter(kpi => kpi.area_id === row.areaId)
                : [];
                ...
                html: `
                        <div style="display: flex; justify-content: space-around; margin: 30px 0px">
                            <div style="align-self: center;">
                                <div>
                                    <strong>Área</strong> <p>${row.areaNombre}</p>
                                </div>
                                <div>
                                    <strong>Descripción</strong> <p> ${row.areaDescripcion}</p>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <strong>Proceso</strong> <p> ${row.procesoNombre}</p>
                                </div>
                                <div>
                                    <strong>Descripción</strong> <p> ${row.procesoDescripcion}</p>
                                </div>
                            </div>    
                        </div>
                ...
                didOpen: () => {
                    ...
                    root.render(
                        <MultipleSelectChip
                            ...
                            KPIS={GETALLKPISADMINISTRATIVOS}
                        />
                    );
                },
            });
    ...
    //Evento Asignar KPI
    const AsignacionStaff = async (row) => {
        ...
        const kpis = Array.isArray(response.data)
            ? response.data
            : response.data.data || [];
        const kpisRelacionados = kpis.filter(kpi => kpi.area_id === row.areaId);
                        ...
                        onSelectionChange={(ids) => {
                            ...
                            if (confirmBtn) {
                                ...
                                if (initialKpiIds.length === 0) {
                                    //Servicio sin asignaciones previas, btn inicial
                        ...
                        if (result2.isConfirmed) {
                            ...
                            try {
                                const response = await apiClient.post(
                                    `http://${Servidor}/API/INSERT-UPDATE/AREAS-KPI/`,
                                    {
                                        area_id: row.areaId,
                                        proceso_id: row.procesoId,
    ...
    const tblAreas = getAreas(verArea, AsignacionStaff)
        ...
        <>
            <div className="d-flex align-items-center">
                <div className="p-2 flex-grow-1 text-muted mb-4">
                    Número de registros:{" "}
                    {filtroStaff && filtroStaff.length && filtroStaff.length}
                </div>
                <div className="p-2">
                    <TextField
                        fullWidth
