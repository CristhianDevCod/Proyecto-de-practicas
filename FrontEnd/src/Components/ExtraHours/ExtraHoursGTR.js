...
import { PERMISSIONSCLIENTSNOVELTIES, getAllListShiftsNowForGtr, getAllPetitionsHourExtra } from '../../API/API';
import { CustomNoRowsOverlay } from '../Notifications/Styles/StylesNotification';
...
const ExtraHoursGTR = () => {
    ...
    useEffect(() => {
        const getListClients = async () => {
            try {
                ...
                setListClientPermissions(response);
            } catch (error) {
                api.error({
    ...
    const getPeticionsHoursExtra = useCallback(async () => {
        setLoading(true);
        try {
            let Cliente = listClientPermissions.map((item) => { return item.Cliente_Permiso });
            const response = await getAllPetitionsHourExtra(Cliente);
            setData(response);
            setLoading(false);
        } catch (error) {
            setLoading(false);
    ...
    useEffect(() => {
        if (listClientPermissions && listClientPermissions.length > 0) {
            getPeticionsHoursExtra();
        }
    }, [getPeticionsHoursExtra, listClientPermissions]);

    // EFECTO PARA OBTENER EL LISTADO DE TURNOS ACTUALES Y MOSTRARSELOS A LOS GTR
    useEffect(() => {
        const getPeticionsHoursExtra = async () => {
            try {
                const response = await getAllListShiftsNowForGtr(now);
                setDataShifts(response);
                setOriginData(response);
            } catch (error) {
                console.error('Error al obtener datos de horas extras:', error);
            }
        };
        getPeticionsHoursExtra();
    }, [Servidor, now]);
    ...
    const handleAcceptHourExtra = () => {
        if (socket && selectedHour) {
            ...
            const Documento_GTR = fullName && fullName.map((data) => data.Documento);
            ...
            socket.emit('acceptHoursExtra', {
                ...
                Documento_Aprobador: Documento_GTR,
    ...
    const ExportHourExtra = async () => {
        ...
        try {
            ...
            let Clientes = listClientPermissions.map((item) => item.Cliente_Permiso);
            const Cliente = Clientes.join(',');
            
            const response = await apiClient.get(`http://${Servidor}/API/GET/DATA-HOURS-EXPORT-ADVISER-FOR-GTR/${month}/${Cliente}`);
            ...
            console.error('Error al exportar las horas extras:', error);
            setLoadingExport(false);
            api.error({
                message: 'Error',
                description: 'Ha ocurrido un error al exportar las horas extras. Por favor selecciona un mes diferente',
            });
        }
    };
                    ...
                    <div className='card border-light mt-3 mb-3 shadow-sm bg-body rounded' style={{ marginLeft: '2%' }}>
                        <div className='card-body'>
                            ...
                            <table className='table table-borderless table-hover table-sm'>
                                ...
                                <tbody>
                                    {Array.isArray(currentPageData) && currentPageData.map((shifts) => {
                                        return (
