...
const SkillFormPopPup = ({ onClose, onSkillSaved, servicioId, servicioNombre, initialDataSkill = { skills: [] }, setSkillData }) => {
 const [skills, setSkills] = useState(initialDataSkill.skills || [{ nombreSkill: '', idSistemaGestion: '', fechaInicio: null, fechaFin: null, estadoSkill: 'activo', error: null }]);
  ...
  useEffect(() => {
    const fetchSistemasGestion = async () => {
      try {
        const response = await apiClient(`http://${Servidor}/sistemasGestionActivos`);
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Stack direction="column" spacing={2} sx={{ maxWidth: 400, margin: '0 auto' }}>
        <form onSubmit={handleSaved}>
          ...
          <Button variant="outlined" style={{ marginTop: '10px' }} onClick={onClose} color="error" fullWidth>
            Cancelar
          </Button>
        </form>
        {alert === 1 && (
        ...
        <Dialog open={openConfirmDialog} onClose={() => setOpenConfirmDialog(false)}>
          ...
          <DialogContent>
            ...
            <DialogContentText>
              Puedes editar toda la información que necesitas antes de guardar
            </DialogContentText>
          </DialogContent>
          <DialogActions>
...
export default SkillFormPopPup;
