...
export default function EditSectorClienteForm({ sectorClienteId, sectorClienteData,  onClose }) {

  const { Servidor } = Service();

  const [nombreSectorCliente, setNombreSectorCliente] = useState('');
  const [estadoSectorCliente, setEstadoSectorCliente] = useState('');
  ...
  useEffect(() => {
    ...
    if (sectorClienteData && sectorClienteData.length > 0) {
      const sectorCliente = sectorClienteData[0];
      setNombreSectorCliente(sectorCliente.nombre_sector_cliente || '');
      setEstadoSectorCliente(sectorCliente.estado_sector_cliente === 1 ? 'activo' : 'inactivo');
    }
  }, [sectorClienteData]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (nombreSectorCliente.trim() === '') {
      setNombreError(true);
      return;
    } else {
      setNombreError(false);
    }
    if (estadoSectorCliente.trim() === '') {
      setEstadoError(true);
      return;
    } else {
      setEstadoError(false);
    }
    const estadoNumerico = estadoSectorCliente === 'activo' ? 1 : 0;

    try {
      const url = `http://${Servidor}/sectorClientes/${sectorClienteId}`;
      const data = {
        nombreSectorCliente: nombreSectorCliente.toUpperCase(),
        estadoSectorCliente: estadoNumerico
      };
  ...
  const handleNombreChange = (e) => {
    ...
    if (regex.test(inputValue) || inputValue === '') {
      setNombreSectorCliente(inputValue);
      setNombreError(false);
    } else {
      setNombreError(true);
    }
  ...
  const handleEstadoChange = (e) => {
    ...
    setEstadoSectorCliente(selectedValue);
