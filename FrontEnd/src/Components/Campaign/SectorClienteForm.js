...
export default function SectorClienteForm({ onClose }) {

  const { Servidor } = Service();

  const [nombreSectorCliente, setNombreSectorCliente] = useState('');
  const [estadoSectorCliente, setEstadoSectorCliente] = useState('activo');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = `http://${Servidor}/sectorClientes`;
      const data = {
    ...
    >
      ...
      {alert === 1 && (
        <Alert severity="success" onClose={() => setAlert(-1)}>
          Sector creada con éxito
        </Alert>
      )}
      {alert === 0 && (
        <Alert severity="error" onClose={() => setAlert(-1)}>
          ¡Ya existe un sector creada con ese mismo nombre!
        </Alert>
      )}
    </Stack>
  );
