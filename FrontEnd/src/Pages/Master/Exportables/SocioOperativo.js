...
const SocioOperativo = ({ XLSX, api, contextHolder }) => {
        ...
        const getListClients = async () => {
            try {
                ...
                if (response.status === 200) {
                    ...
                    api.error({
                        message: 'No se puedo obtener los permiso para el exporte del socio operativo'
                    })
                }
            } catch (error) {
                api.error({
    ...
    const exportToXLSX = async () => {
        ...
        try {
            ...
            const response = await apiClient.get(`http://${Servidor}/API/EXPORTS/DATA/SOCIODEMOGRAPHY-OP/${mes}/${Cliente}`);
            ...
            if (response.length === 0) {
                ...
                const fullFileName = `Sociodemográfico Operativo.xlsx`;
            ...
            <div className='d-flex'>
                <div className='p-2 flex-grow-1 align-self-center'>
                    <div className='title-exports-shifts'>
                        <i className='bi bi-grid-fill' id='icons-files-exports' /> Sociodemográfico Operativo
                    </div>
                </div>
                <div className='p-2'>
                    <input
