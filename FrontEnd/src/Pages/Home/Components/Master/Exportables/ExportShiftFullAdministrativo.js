...
const ExportShiftFullAdministrativo = ({ api, XLSX, fullName }) => {
    ...
    const handleExportClick = async () => {
        setIsLoading(true);
        try {
            const response = await apiClient.get(`http://${Servidor}/API/GET-EXPORT/SHIFTS-ADMINISTARTIVOS/v2?startDate=${startDate}&endDate=${endDate}`, {
                headers: {
            ...
            const fullFileName = `Turnos Completos Administrativo.xlsx`;
            ...
            <div className='d-flex'>
                <div className='p-2 flex-grow-1 align-self-center'>
                    <div className='title-exports-shifts'>
                        <i className='bi bi-calendar-check-fill color-icon-shift' /> Turnos Completos Administrativos
                    </div>
                </div>
...
export default ExportShiftFullAdministrativo