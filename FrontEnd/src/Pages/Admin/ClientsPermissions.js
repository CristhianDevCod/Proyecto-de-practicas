...
import { User_Permissions, getAllListClient } from '../../API/API';
...
const ClientsPermissions = () => {
    ...
    const { InsertAndDeletePermissionsClients } = Config();
    ...
    const getUserPermissions = async (usuarioRed) => {
        const url = `http://${Servidor}/API/GET-USER-PERMISSIONS-CLIENT/${usuarioRed}`;
        ...
        try {
            ...
            if (response.status === 200) { // Verifica si el estado de la respuesta es 200 OK
                ...
                clearPermissions();
                // Maneja el caso donde el usuario no tiene permisos asignados
                api.error({
                    message: `El usuario ${usuarioRed} no tiene permisos asignados`,
                    description: `Estado de la respuesta: ${response.status} - ${response.statusText}`
                });
            }
        } catch (error) {
            clearPermissions();
    ...
    const savePermissions = async () => {
        try {
            ...
            // console.log(permisosMarcados);

            // // Verifica los permisos a eliminar (desmarcados)
            const permissionsDelete = clientePermiso.filter((permiso) => !permisosMarcados.includes(permiso));
            // console.log('Permisos a eliminar', permissionsDelete);

            // // Verifica los permisos a insertar (marcados)
            // const permissionsInsert = permisosMarcados.filter((cliente) => clientePermiso.includes(cliente));
            // console.log('Permisos a insertar', permissionsInsert);

            if (action === 'delete') {
                // Realiza la eliminación de permisos desmarcados
                const response = await apiClient.put(InsertAndDeletePermissionsClients, {  
                        action: action,
                ...
                // Llama a la función selectAllPermissions para insertar todos los permisos seleccionados
                selectAllPermissions();

                // Realiza la inserción de permisos marcados
                const response = await apiClient.put(InsertAndDeletePermissionsClients, {
                        action: action,
    ...
    return (
        <>
            ...
            <div className='text-center'>
                <div className='mb-0 title-import'>Permisos de clientes</div>
            </div>
